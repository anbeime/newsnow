...
function formatUrl(
  ...
  baseUrl: string = "https://www.freebuf.com",
): string {
  if (!url) return "";
...
function extractCategory($article: cheerio.Cheerio<any>): string {
  // 从URL路径推断分类
  const articleUrl =
    $article.find(".title-left .title").parent().attr("href") || "";
...
export default defineSource(async () => {
  console.log("开始获取 Freebuf 数据...");
  try {
    const baseUrl = "https://www.freebuf.com";
    const html: any = await myFetch<any>(baseUrl, {
      headers: {
        "User-Agent":
    ...
    console.log("Freebuf 解析到", articles.length, "篇文章");
